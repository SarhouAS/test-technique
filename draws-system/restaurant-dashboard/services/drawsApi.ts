...
// Nouvelle interface pour la réponse complète de GET /draws/:id
...
export class DrawsApiService {
  ...
  async getDrawDetail(drawId: string): Promise<DrawDetailResponse> {
    try {
      ...
      return response.data.data; // Retourne la réponse complète
    } catch (error) {
      throw this.handleError(error);
    }
  }
...
export function getDrawsApi(): DrawsApiService {
  if (!apiService) {
    ...
    console.warn("DrawsApi not initialized. Using placeholder URL.");
    initializeDrawsApi('http://localhost:3001/api/v1');
  }
  return apiService!;
}